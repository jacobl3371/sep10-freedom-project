...
I paused in my engineering design process since I took the time to research and tinker with different tools until finding TailwindCSS and getting it to work. I did do some research on crypto currency and its compatability with the only OS which works with that the VPN for this project, Tails since I configured a new wallet seed in Electrum. I found that more specifically than the OS, the actual stock, approved cryptocurrency application for Tails, ELectrum Bitcoin, is named so becuase it is only a Bitcoin wallet and can not hold any other crypto currencies. I was under the false assumption until a couple months ago that I could only buy whole crypto currencies, so I had to settle for some obscure crypto which I didn't even know wasn't compatible with any secured trustworthy wallet on the operating system that every other aspect of this project relies on. Coming to the conclusion that I could buy fractions of a crypto coin, I intended to buy a more trustworthy and known crypto currency like Eteruem, before I was doing some research on transferring crypto to Electrum Bitcoin and watched [this video]() which guided me on how to send Bitcoin from an online, account-based custodial wallet, to your non-custodial, user-controlled Electrum Bitcoin wallet. The video gave me a lot of insight and the whole thing is a pretty easy, quick, graphical-user-interface-enabled setup which I will be going over in Blog 5.
